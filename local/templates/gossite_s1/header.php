...
// is main page
$IS_MAIN = 'N';
if ($requestPage === SITE_DIR . 'index.php') {
    $IS_MAIN = 'Y';
}
...
if ($USER->IsAuthorized()) {
    ...
    if (in_array(9, $arGroups) || in_array(10, $arGroups) || in_array(11, $arGroups)) {
        if (in_array(9, $arGroups)) $S = '(Р)';
        elseif (in_array(10, $arGroups)) $S = '(НО)';
        elseif (in_array(11, $arGroups)) $S = '(И)';
        ?>
        <div class="personal-header-link">
            <a href="/personal/"><?= $USER->GetFullName() ?> <?= $S ?></a>
            <span id="personal-header-badges">
                </span>
            <a href='<?= $APPLICATION->GetCurPage() . '?' . \CUser::getLogoutParams() ?>'>Выйти</a>
        </div>
        <?
    }
    else {
