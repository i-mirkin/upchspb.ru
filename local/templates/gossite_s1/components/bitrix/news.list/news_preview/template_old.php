...
<?if(!empty($arResult["ITEMS"])) { ?>
	...
	<?foreach($arResult["ITEMS"] as $arItem):?>	
		...
		?>
		<a href="<?=$arItem["DETAIL_PAGE_URL"]?>"  class="news-list__item <?if($arItem['DISPLAY_PROPERTIES']['BREAKING_NEWS']['VALUE'] == 'Y') { ?>news-list__item--main<? } ?> clearfix" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
			...
			<?if(!$fileImg) { ?>				
				<div class="news-list__item-img" style="background-image: url(<?=SITE_TEMPLATE_PATH?>/images/noimg.png"></div>
			<? } else {?>
				<div class="news-list__item-img" style="background-image: url(<?=$fileImg['src']?>);"></div>
			<? } ?>			
