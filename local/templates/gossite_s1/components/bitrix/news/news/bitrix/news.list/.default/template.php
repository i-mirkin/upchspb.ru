...
<?if(!empty($arResult["ITEMS"])) { ?>
	...
	<?foreach($arResult["ITEMS"] as $arItem):?>				
		...
		?>
		<div class="news-list__item <?if($arItem['DISPLAY_PROPERTIES']['THEME_STATUS']['VALUE_ENUM_ID'] == 44) { ?>news-list__item--main<? } ?> clearfix" id="<?=$this->GetEditAreaId($arItem['ID']);?>" <?if('Y' == $arItem['PROPERTIES']['ADMIN_ONLY']['VALUE']) { ?>data-admin-only="Y"<? } ?>>
			<div class="news-list__item-top clearfix">
				...
				<? } ?>
				<?if($arItem['DISPLAY_PROPERTIES']['THEME_STATUS']['VALUE_ENUM_ID'] == 44) { ?>
