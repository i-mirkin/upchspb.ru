...
\Bitrix\Main\Diag\Debug::writeToFile(['test' => 'ok'],'','/schedule_log_test.txt');
\Bitrix\Main\Diag\Debug::writeToFile(['test' => 'ok'],'','/local/log/schedule_log_1.txt');
	...
	if(isset($resScheduleBaseList[$period['name']]) && is_array($resScheduleBaseList[$period['name']])) {
		foreach($resScheduleBaseList[$period['name']] as $baseEl) {
			...
			$currentScheduleProp = [
				...
				'NAME' => $csnameEl,
				'ACTIVE' => 'Y',
				'CODE' => CUtil::translit($csnameEl, 'ru'),
				'PROPERTY_VALUES' => [
