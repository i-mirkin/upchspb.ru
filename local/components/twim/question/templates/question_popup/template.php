...
<div class="mfeedback">
    ...
    <form name="question_form" action="<?=POST_FORM_ACTION_URI?>" method="POST" class="question_form"  accept-charset="windows-1251">
        ...
        <?if(!empty($arResult['arFieldsCateg'])) { ?>
            <div class="form-group">
                <p><?=GetMessage('MFT_CATEG')?></p>
                <select name="q_categ">
                    <option selected value=""><?=GetMessage('MFT_CATEG_CHOOSE')?></option>
                    <?foreach($arResult['arFieldsCateg'] as $arField) { ?>
        ...
        <script>
            jQuery('.question_form').append('<input type="hidden" name="qcapt" value="Y">');
            jQuery(".question_form .form-group select").styler();


            // Проверка js проверка полей в init_build.js
