...
if($this->InitComponentTemplate($templatePage))
{
	...
	if($folderPath <> '')
	{
		...
		$allResList = [];
		$obSearchAll = new CSearch();
		$arFilterAll = $arFilter;
		$aSortAll = $aSort;
		$exFILTERAll = $exFILTER;
		//dump($arFilter);
		//dump($exFILTERAll);
		//When restart option is set we will ignore error on query with only stop words
		$obSearchAll->SetOptions(array(
		    "ERROR_ON_EMPTY_STEM" => $arParams["RESTART"] != "Y",
		    "NO_WORD_LOGIC" => $arParams["NO_WORD_LOGIC"] == "Y",
		));
		
		$obSearchAll->Search($arFilterAll, $aSortAll, $exFILTERAll);
		
		if($obSearchAll->errorno==0)
		{
		    $obSearchAll->NavStart((int)COption::GetOptionInt("search", "max_result_size"), false);
		    $arAll = $obSearchAll->GetNext();
		    //Search restart
		    if(!$arAll && ($arParams["RESTART"] == "Y") && $obSearchAll->Query->bStemming)
		    {
		        $exFILTERAll["STEMMING"] = false;
		        $obSearchAll = new CSearch();
		        $obSearchAll->Search($arFilterAll, $aSortAll, $exFILTERAll);
		    ...
		    while($arAll)
		    {
		        $allResList[$arAll['ID']] = strtotime($arAll['FULL_DATE_CHANGE']);
		        $arAll = $obSearchAll->GetNext();
		    }
		    //dump($allResList);   
		}
		//dump(ConvertTimeStamp(min($allResList),'FULL'));
		//dump(ConvertTimeStamp(max($allResList),'FULL'));		
		/*$arFilter[">=DATE_CHANGE"] = ConvertTimeStamp(min($allResList),'SHORT');
		 $arFilter["<=DATE_CHANGE"] = ConvertTimeStamp(max($allResList),'SHORT');*/
		
		
		
		//if(empty($arResult["REQUEST"]["~FROM"])) $arResult["REQUEST"]["~FROM"] = ConvertTimeStamp(min($allResList),'FULL');
		//if(empty($arResult["REQUEST"]["~TO"])) $arResult["REQUEST"]["~TO"] = ConvertTimeStamp(max($allResList),'FULL');

		// DATE FILTER

		if(is_object($arResult['NAV_RESULT']) && get_class($arResult['NAV_RESULT']) == 'CSearch') {
		    $arResult['RES_COUNT'] = $arResult['NAV_RESULT']->SelectedRowsCount();
		    $arResult['RES_COUNT_DESC'] = num_decline( $arResult['RES_COUNT'], GetMessage("SEARCH_RESULTS"));
		}
