...
if($this->startResultCache(false, array(($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()), $bUSER_HAVE_ACCESS, $arNavigation, $arrFilter, $pagerParameters)))
{
	...
	while($obElement = $rsElement->GetNextElement())
	{
		...
		{
			if ($arItem["PROPERTIES"][$pid[0]] && (!$pid[1] || $arItem["PROPERTIES"][$pid[0]]["IBLOCK_ID"] == $pid[1]))
			{
				$prop = &$arItem["PROPERTIES"][$pid[0]];
				if(
					(is_array($prop["VALUE"]) && count($prop["VALUE"])>0)
					|| (!is_array($prop["VALUE"]) && strlen($prop["VALUE"])>0)
				)
				{
					$arItem["DISPLAY_PROPERTIES"][$pid[0]] = CIBlockFormatProperties::GetDisplayValue($arItem, $prop, "news_out");
				}
			}
		}
