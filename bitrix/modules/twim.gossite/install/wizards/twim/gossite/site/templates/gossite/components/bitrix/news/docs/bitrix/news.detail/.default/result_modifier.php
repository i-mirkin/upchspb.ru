...
if(is_array($arResult["DISPLAY_PROPERTIES"]["FILE"]["VALUE"])
    && count($arResult["DISPLAY_PROPERTIES"]["FILE"]["VALUE"]) > 1
):
...
//NOT_DISCUSSION для мун услуг и раздела проекты
if($arResult["IBLOCK_TYPE_ID"] == "mun_servise"
    && $arResult["SECTION"]["PATH"][0]["CODE"] == "draft-regulations"
){
    $now = new DateTime("now");
    $datePub = date_create($arResult["ACTIVE_FROM"]);
    $interval = $now->diff($datePub);
    if($interval->m > 0){
        $arParams["NOT_DISCUSSION"] = "Y";
    }
}