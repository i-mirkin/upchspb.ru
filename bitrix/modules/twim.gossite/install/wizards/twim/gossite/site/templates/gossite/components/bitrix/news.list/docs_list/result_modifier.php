...
foreach($arResult["ITEMS"] as $arItem){
    $arSecID[] = $arItem["IBLOCK_SECTION_ID"];
}
$arSectionID = array_unique($arSecID);
// получаем названия по id
$arSection = array();
$arFilter = Array('IBLOCK_ID'=>$arItem["IBLOCK_ID"], 'ID' => $arSectionID);
$db_list = CIBlockSection::GetList(Array("SORT"=>"ASC"), $arFilter, false, array("ID", "NAME", "SECTION_PAGE_URL"), false);
...
if(!function_exists('get_service_viewer')){
/**
 * get link viewer
 * @param type $file
 * @return type
...
foreach($arResult["ITEMS"] as $key => $arItem){
    $doc_count = 0;
    if(isset($arItem["DISPLAY_PROPERTIES"]["FILE"]["VALUE"]) && is_array($arItem["DISPLAY_PROPERTIES"]["FILE"]["VALUE"])){
        $doc_count = count($arItem["DISPLAY_PROPERTIES"]["FILE"]["VALUE"]);
    }
    if(!$doc_count)  continue;
    $arFiles = $arItem["DISPLAY_PROPERTIES"]["FILE"]["FILE_VALUE"];
    if($doc_count > 1):
        foreach ($arFiles as $id => $file){
            $item = &$arResult["ITEMS"][$key]["DISPLAY_PROPERTIES"]["FILE"]["FILE_VALUE"][$id];
            $item["FORMAT_SIZE"] = format_size($file["FILE_SIZE"]);  
            $item["LINK"] = get_service_viewer ($file["SRC"]);   
            $item["ICON"] = get_icon_file ($file["SRC"]);
        }
    else:
        $item = &$arResult["ITEMS"][$key]["DISPLAY_PROPERTIES"]["FILE"]["FILE_VALUE"];
