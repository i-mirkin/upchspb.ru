...
<?foreach($arResult["ITEMS"] as $arItem):?>
    ...
    <div class="item"  id="<?=$this->GetEditAreaId($arItem['ID']);?>">
        ...
        <?endif?>
        <?if($arResult["SECTION"][$arItem["IBLOCK_SECTION_ID"]]):?>
            <p class="section">
                <a href="<?=$arResult["SECTION"][$arItem["IBLOCK_SECTION_ID"]]["SECTION_PAGE_URL"]?>"><?echo $arResult["SECTION"][$arItem["IBLOCK_SECTION_ID"]]["NAME"]?></a>
            </p>
        <?endif?>  
        <?if($arParams["DISPLAY_NAME"]!="N" && $arItem["NAME"]):?>
            <?if(!$arParams["HIDE_LINK_WHEN_NO_DETAIL"] || ($arItem["DETAIL_TEXT"] && $arResult["USER_HAVE_ACCESS"])):?>
                <a href="<?echo $arItem["DETAIL_PAGE_URL"]?>"  class="title-doc"><b><?echo $arItem["NAME"]?></b></a>
            <?else:?>
                <p class="title-doc">
